<template>
  <div class="list-container">
    <div class="list head">
      <div class="title"><p>標題</p></div>
      <div class="content"><p>內容預覽</p></div>
      <div class="action"><p>編輯</p><p>查看</p><p>刪除</p></div>
    </div>
    <div class="list" v-for="(item, i) in news.data" :key="i">
      ...
      <div class="action">
        <p @click="$router.push(`news/edit/${item._id}`)"><font-awesome-icon icon="edit" /></p>
        <p @click="openView(item._id)"><font-awesome-icon icon="eye" /></p>
        <p @click="deleteAction(item._id)"><font-awesome-icon icon="minus-circle" /></p>
      </div>
    </div>
  </div>
</template>
